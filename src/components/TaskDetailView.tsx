              ...
              return (
                <div key={step.key} className="flex items-center flex-1 min-w-0">
                  <button
                    ...
                    <span className="text-[10px] font-extrabold truncate" style={{ WebkitFontSmoothing: 'antialiased' }}>{step.label}</span>
                  </button>
                  {i < arr.length - 1 &&
...
{localDeadline && (() => {const now = new Date();const diff = Math.ceil((localDeadline.getTime() - now.getTime()) / (1000 * 60 * 60 * 24));if (diff < 0) return <span className="text-red-500 text-[10px] ml-1">(срок сдачи просрочен)</span>;if (diff === 0) return <span className="text-yellow-500 text-[10px] ml-1">(сегодня)</span>;const dLabel = diff === 1 ? "день" : diff < 5 ? "дня" : "дней";const colorClass = diff <= 3 ? "text-yellow-500" : "text-foreground/70";return <span className={`${colorClass} text-[10px] ml-1`}>(осталось {diff} {dLabel})</span>;})()}
            ...
            <div key={cl.id} className="rounded-xl border border-border bg-card px-3 py-2">
            ...
            {localStatus === "approval" && <div className="rounded-xl border border-border bg-background dark:bg-muted/50 px-3 py-2">
              <div className="flex items-center gap-2 mb-1.5">
