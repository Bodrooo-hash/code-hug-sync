...
const Sidebar = () => {
  ...
  const pageItems: NavItem[] = [
    { label: "Мой аккаунт", icon: <User size={18} />, chevron: true },
  ...
  const renderItem = (item: NavItem) => {
    ...
    return (
      ...
      >
        {item.dot && !isCollapsed && <span className="mr-[-4px] h-1.5 w-1.5 rounded-full bg-foreground opacity-25" />}
        {item.dot && isCollapsed && <span className="h-1.5 w-1.5 rounded-full bg-foreground opacity-25" />}
        {item.icon && (
        ...
        {!isCollapsed && item.chevron && <ChevronRight size={14} className="text-muted-foreground" />}
      </button>
    );
  };
  ...
  return (
    <aside className={`fixed left-0 top-0 flex h-screen flex-col border-r border-border bg-card transition-all duration-200 ${isCollapsed ? "w-[60px]" : "w-[var(--sidebar-width)]"}`}>
      <div className={`flex items-center gap-3 bg-card ${isCollapsed ? "justify-center px-2 py-5" : "px-6 py-5"}`}>
        ...
        {!isCollapsed && <span className="text-xs font-normal">Бодренков К.</span>}
      </div>
      ...
      <nav className={`flex-1 space-y-4 overflow-y-auto bg-card ${isCollapsed ? "px-1.5 py-2" : "px-3 py-2"}`}>
        ...
        <div>
          {!isCollapsed && <p className="mb-2 px-3 text-xs text-muted-foreground opacity-50">Страницы</p>}
