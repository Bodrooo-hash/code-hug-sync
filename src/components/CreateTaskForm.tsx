...
import { createTask, type Bitrix24User } from "@/lib/bitrix24";
...
/* ─── Inline user picker (single) ─── */
const UserPicker = ({ label, icon: Icon, members, selectedId, onSelect, error }: {label: string;icon: React.ElementType;members: Bitrix24User[];selectedId: string;onSelect: (id: string) => void;error?: boolean;}) => {
  ...
  return (
    <div className="flex items-center gap-3 py-2" ref={ref}>
      ...
      <div className="relative flex-1 min-w-0">
        <div onClick={() => setOpen((v) => !v)} className="flex items-center gap-2 w-full px-2.5 py-1.5 cursor-pointer text-left min-h-[32px]">
          {selected ? (
            <div className="flex items-center gap-2 flex-1 min-w-0">
              <Avatar className="w-6 h-6 shrink-0"><AvatarImage src={selected.PERSONAL_PHOTO} /><AvatarFallback className="text-[8px] bg-muted">{getInitials(fullName(selected))}</AvatarFallback></Avatar>
              <span className="text-xs text-foreground/70 truncate">{fullName(selected)}</span>
              <button onClick={(e) => {e.stopPropagation();onSelect("");}} className="ml-auto shrink-0 w-4 h-4 flex items-center justify-center rounded-full hover:bg-foreground/[0.08]"><X className="w-3 h-3 text-foreground/30" /></button>
            </div>
          ) : (
        ...
        {open && (
          <div className="absolute left-0 top-full mt-1 z-50 w-full max-h-40 overflow-auto rounded-lg border border-border bg-card shadow-lg p-1 animate-in fade-in-0 zoom-in-95 duration-150">
            {members.length === 0 ? <p className="text-xs text-muted-foreground italic px-2 py-1">Нет участников</p> :
              members.map((u) => (
                <button key={u.ID} onClick={() => {onSelect(u.ID);setOpen(false);}} className={`flex items-center gap-2 w-full px-2 py-1.5 rounded-md transition-colors text-left ${selectedId === u.ID ? "bg-ring/10" : "hover:bg-foreground/[0.04]"}`}>
                  <Avatar className="w-6 h-6"><AvatarImage src={u.PERSONAL_PHOTO} /><AvatarFallback className="text-[8px] bg-muted">{getInitials(fullName(u))}</AvatarFallback></Avatar>
                  <span className="text-xs text-foreground/70 truncate">{fullName(u)}</span>
                  {selectedId === u.ID && <span className="ml-auto text-[10px] text-ring font-medium">✓</span>}
                </button>
              ))}
          </div>
        )}
...
/* ─── Multi user picker ─── */
const MultiUserPicker = ({ label, icon: Icon, members, selectedIds, onToggle }: {label: string;icon: React.ElementType;members: Bitrix24User[];selectedIds: string[];onToggle: (id: string) => void;}) => {
  const [open, setOpen] = useState(false);
  const ref = useRef<HTMLDivElement>(null);
      ...
      <div className="relative flex-1 min-w-0">
          ...
          {selected.length > 0 ? (
            <div className="flex items-center gap-2 flex-1 min-w-0">
              <div className="flex items-center -space-x-1.5">
                {selected.slice(0, 3).map((u) => <Avatar key={u.ID} className="w-6 h-6 shrink-0 border border-card"><AvatarImage src={u.PERSONAL_PHOTO} /><AvatarFallback className="text-[8px] bg-muted">{getInitials(fullName(u))}</AvatarFallback></Avatar>)}
              </div>
              <span className="text-xs text-foreground/70 truncate">{selected.length === 1 ? fullName(selected[0]) : `${selected.length} чел.`}</span>
              <button onClick={(e) => {e.stopPropagation();selected.forEach((u) => onToggle(u.ID));}} className="ml-auto shrink-0 w-4 h-4 flex items-center justify-center rounded-full hover:bg-foreground/[0.08]"><X className="w-3 h-3 text-foreground/30" /></button>
            </div>
          ) : (
            <div className="flex items-center gap-2 flex-1 min-w-0 group/pick">
              <User className="w-4 h-4 text-foreground/30 group-hover/pick:text-ring transition-colors" />
        ...
        {open && (
          <div className="absolute left-0 top-full mt-1 z-50 w-full max-h-40 overflow-auto rounded-lg border border-border bg-card shadow-lg p-1 animate-in fade-in-0 zoom-in-95 duration-150">
            {members.length === 0 ? <p className="text-xs text-muted-foreground italic px-2 py-1">Нет участников</p> :
              members.map((u) => (
                <button key={u.ID} onClick={() => { onToggle(u.ID); setOpen(false); }} className={`flex items-center gap-2 w-full px-2 py-1.5 rounded-md transition-colors text-left ${selectedIds.includes(u.ID) ? "bg-ring/10" : "hover:bg-foreground/[0.04]"}`}>
                  <Avatar className="w-6 h-6"><AvatarImage src={u.PERSONAL_PHOTO} /><AvatarFallback className="text-[8px] bg-muted">{getInitials(fullName(u))}</AvatarFallback></Avatar>
                  <span className="text-xs text-foreground/70 truncate">{fullName(u)}</span>
                  {selectedIds.includes(u.ID) && <span className="ml-auto text-[10px] text-ring font-medium">✓</span>}
                </button>
              ))}
          </div>
        )}
...
const CreateTaskForm = ({ projectId, projectName, sectionName, members, onBack, onCreated }: Props) => {
  const { data: profile } = useBitrix24Profile();
  ...
  useEffect(() => { if (profile?.ID && !creatorId) { setCreatorId(profile.ID); } }, [profile, creatorId]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      await createTask({
        title: title.trim(), description: description.trim() || undefined, groupId: projectId,
        responsibleId: responsibleId || undefined, deadline: deadline ? format(deadline, "yyyy-MM-dd'T'HH:mm:ss") : undefined,
        accomplices: accompliceIds.length > 0 ? accompliceIds : undefined, auditors: auditorIds.length > 0 ? auditorIds : undefined
      });
      onCreated();
    ...
    <div className="flex flex-col h-full -m-4">
      ...
      <div className="flex-1 min-h-0 overflow-auto px-5 py-5 space-y-4">
        ...
        <div className="rounded-xl border border-foreground/[0.06] bg-foreground/[0.03] px-3">
          <UserPicker label="Исполнитель" icon={User} members={members} selectedId={responsibleId} onSelect={setResponsibleId} error={showValidation && !responsibleId} />
          <MultiUserPicker label="Помощники" icon={Users} members={members} selectedIds={accompliceIds} onToggle={toggleAccomplice} />
          <UserPicker label="Постановщик" icon={UserCheck} members={members} selectedId={creatorId} onSelect={setCreatorId} error={showValidation && !creatorId} />
          <MultiUserPicker label="Наблюдатели" icon={Eye} members={members} selectedIds={auditorIds} onToggle={toggleAuditor} />
