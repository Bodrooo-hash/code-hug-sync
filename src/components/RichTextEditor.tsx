...
import {
  ...
  Paperclip, AtSign, Maximize2, ListOrdered as ListOrderedIcon, Quote,
} from "lucide-react";
...
const RichTextEditor = ({ value, onChange, placeholder, onAttach, onMention, onExpand }: Props) => {
  ...
  return (
    <div className="rounded-xl border border-foreground/[0.06] bg-foreground/[0.03] overflow-visible relative">
      ...
      <div className="flex items-center gap-0.5 px-3 py-1.5 border-t border-foreground/[0.06]">
        ...
        <button type="button" title="Развернуть" onClick={onExpand} className="w-6 h-6 flex items-center justify-center rounded-md text-foreground/30 hover:text-foreground/60 hover:bg-foreground/[0.04] transition-colors"><Maximize2 className="w-3 h-3" /></button>
      </div>
    </div>
  );
};
