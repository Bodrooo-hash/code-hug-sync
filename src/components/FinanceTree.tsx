...
import { fetchGroupTasks, type Bitrix24Task } from "@/lib/bitrix24";
...
const sections: Section[] = [
  { key: "ops", roman: "I", title: "Операционные финансы", abbr: "OF", icon: <Banknote className="w-4 h-4" />, color: "#3b82f6", items: [{ id: 46, name: "Исходящие " }, { id: 52, name: "Входящие " }] },
  ...
  { key: "mgmt", roman: "IV", title: "Управленческий учёт ", abbr: "UU", icon: <BarChart3 className="w-4 h-4" />, color: "#3b82f6", items: [{ id: 48, name: "Фин. планирование " }, { id: 66, name: "Отчёты" }] },
  ...
  { key: "legal", roman: "VI", title: "Документы", abbr: "D", icon: <Scale className="w-4 h-4" />, color: "#3b82f6", items: [{ id: 68, name: "Маркетплейсы: Сверка взаиморасчетов" }, { id: 70, name: "B2B: Сверка с контрагентами" }, { id: 72, name: "Розничные продажи: Сверка эквайринга, ККД" }, { id: 74, name: "Розничные продажи: Обработка возвратов" }, { id: 76, name: "Тендеры, гос. и корп. закупки" }] },
];
...
function SectionBranch({ section, defaultExpanded = true, totalTasks, doneTasks, loading, itemStats, onSelectItem, selectedProjectId }: {
  section: Section; defaultExpanded?: boolean; totalTasks: number; doneTasks: number; loading?: boolean;
  itemStats: Record<number, ItemTaskStats>; onSelectItem?: (item: ProcessItem | null, sectionTitle: string) => void; selectedProjectId?: number | null;
}) {
          ...
          {section.items.map((item) => {
            ...
            return (
              <div key={item.id} className="relative flex items-center gap-2 py-[3px] group/item">
                ...
                <span className={`text-xs flex-shrink-0 tabular-nums w-5 text-right ${selectedProjectId === item.id ? "text-blue-500" : "text-muted-foreground"}`}>{item.id}</span>
                ...
                {stats && !loading && (
                  <>
                    <span className="text-[11px] font-medium text-muted-foreground tabular-nums bg-transparent px-1 py-0.5 rounded border border-border" title="Всего задач">{stats.total - (stats.done ?? 0)}</span>
                    <div className="ml-auto flex items-center gap-1.5 flex-shrink-0">
...
const FinanceTree = ({ onSelectProject, selectedProjectId }: { onSelectProject?: (project: { id: number; name: string; sectionTitle: string } | null) => void; selectedProjectId?: number | null; }) => {
  ...
  useEffect(() => {
    ...
    const fetchAll = async () => {
      ...
      perItem[46] = { total: 12, done: 7, newTasks: 4, inProgress: 3, consultation: 3, approval: 2, overdue: true };
      if (!results["ops"]) results["ops"] = { total: 0, done: 0 };
      results["ops"].total = Math.max(results["ops"].total, 12);
      results["ops"].done = Math.max(results["ops"].done, 7);
