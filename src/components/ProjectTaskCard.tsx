...
import { fetchGroupTasks, fetchGroupMembers, type Bitrix24Task, type Bitrix24User } from "@/lib/bitrix24";

export interface SelectedProject { id: number; name: string; sectionTitle: string; }
...
const ProjectTaskCard = ({ project, onBack, onTaskClick }: Props) => {
  ...
  const filterCounts = useMemo(() => ({
    all: tasks.length,
    new: tasks.filter((t) => t.status === "1").length,
    inProgress: tasks.filter((t) => t.status === "3").length,
    help: tasks.filter((t) => t.status === "2").length,
    approval: tasks.filter((t) => t.status === "4").length
  }), [tasks]);

  const filteredTasks = useMemo(() => {
    let list = tasks;
    if (activeFilter === "new") list = list.filter((t) => t.status === "1");
    else if (activeFilter === "inProgress") list = list.filter((t) => t.status === "3");
    else if (activeFilter === "help") list = list.filter((t) => t.status === "2");
    else if (activeFilter === "approval") list = list.filter((t) => t.status === "4");
